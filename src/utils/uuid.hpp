...
public:
   ...
   */
  [[nodiscard]] static std::optional<Uuid> parse_safe(const std::span<const char> s) noexcept
  {
    uuid_t uuid;
    if (uuid_parse_range(s.begin().base(), s.end().base(), uuid)) return std::nullopt;
    return uuid;
  }
  ...
  /**
   * Default constructor.
   * Initializes the UUID to the "NULL UUID" (all zeros).
   */
  Uuid() noexcept { uuid_clear(this->uuid_); };

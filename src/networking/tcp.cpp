...
Tcp::Tcp(int _port, const std::function<http::Response(http::Request)> _handle_client)
    : running_(false), handle_client_(_handle_client), addr_(AF_INET, htons(_port), in_addr(htonl(INADDR_ANY)))
{
  // socket
...
void Tcp::client_event(const bool _closed, const Client* _client)
{
  ...
  _client->write(this->handle_client_(_client->read()));
